...
function stream(data, res, header) {
    return !0 === setHeaders(res, "Content-Type", header) ? res.send(data) : null;
}
...
function Sitemap(options) {
    var resolve = require("path").resolve, opt = options || Object.create(null), http = "https" == opt.http ? "https://" : "http://", url = String(opt.url || "127.0.0.1"), port = isNaN(opt.port) ? "" : ":" + Number(opt.port), hideByRegex = !1;
    if (Array.isArray(opt.hideByRegex) && opt.hideByRegex.length > 0) {
        var hideByRegexArray = opt.hideByRegex, hideByRegexLength = hideByRegexArray.length;
        hideByRegex = function(route) {
            for (var i = 0; i < hideByRegexLength; ++i) try {
                if (hideByRegexArray[i].test(route)) return !0;
            } catch (_) {
                continue;
            }
            return !1;
...
module.exports = sitemap, Sitemap.prototype.generate = function(app, router, store) {
    ...
    for (var map = Object.create(null), routing = app._router ? app._router.stack : app.stack, i = 0, ii = routing.length; i < ii; ++i) {
        if ((route = routing[i]).route) (route = routing[i].route) && route.methods && (route.methods.get || route.methods._all) && (map[route.path.toString()] = [ "get" ]); else if (route.handle && route.handle.stack && router) {
            for (var handle, route, j = 0, jj = router.length; j < jj; ++j) if (route.regexp.test(router[j])) {
                handle = router[j];
                break;
            }
            if (handle) if ("/" === handle && (handle = ""), route = this.generate4(route.handle, router, !1)) for (j = 0, 
            jj = (route = Object.keys(route)).length; j < jj; ++j) map[handle + route[j]] = [ "get" ];
        }
    }
    if (!1 !== store) for (routing in map) this.map[routing] = map[routing];
    return map;
    ...
    return !1 !== store && (this.map = map), map;
    ...
    var route = this.my.route, hideByRegex = this.my.hideByRegex, sitemap = this.map, head = '<?xml version="1.0" encoding="UTF-8"?>';
    head += '<urlset xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"', head += ' xsi:schemaLocation="http://www.sitemaps.org/schemas/sitemap/0.9 http://www.sitemaps.org/schemas/sitemap/0.9/sitemap.xsd http://www.w3.org/TR/xhtml11/xhtml11_schema.html http://www.w3.org/2002/08/xhtml/xhtml1-strict.xsd"', 
    head += ' xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"';
    var langFlag = !0, data = "";
    for (var uri in sitemap) {
        var rr = route.ALL || route[uri] || !1;
        if ((!1 === hideByRegex || !hideByRegex(uri)) && (!rr || !rr.disallow && !rr.hide)) {
            ...
            rr.alternatepages)) {
                ...
                for (var position in pages) {
                    for (var attribute in data += "<xhtml:link", pages[position]) "rel" === attribute && (data += ' rel="' + pages[position][attribute] + '"'), 
    ...
    return !0 === langFlag && (head += ' xmlns:xhtml="http://www.w3.org/TR/xhtml11/xhtml11_schema.html">'), 
    head + data + "</urlset>";
}, Sitemap.prototype.txt = function() {
    var temp = !0, route = this.my.route, hideByRegex = this.my.hideByRegex, sitemap = this.map, data = "User-agent: *\n";
    for (var uri in sitemap) {
        var rr = route[uri];
        ...
        if (rr) {
            if (!1 !== hideByRegex && hideByRegex(uri)) continue;
            rr.disallow && !rr.hide ? (temp = !1, data += "Disallow: " + uri + "\n") : rr.allow && (temp = !1, 
            data += "Allow: " + uri + "\n");
        }
    }
